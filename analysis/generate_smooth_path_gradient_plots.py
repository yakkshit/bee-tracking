"""
Generate path-length normalized smooth two-color gradient trajectory plots:

Normalizes color by cumulative arc length along the trajectory (0% -> 100% path progress),
eliminating time-clustering artifacts where bees staying near the feeder create blocky colors.

Outputs:
  - results/<session>/plots/gradient_full_trajectory_smooth.png
  - results/<session>/plots/gradient_exit_trajectory_smooth.png
"""
import os
import re
import numpy as np
...
TWO_COLOR_CMAP = mcolors.LinearSegmentedColormap.from_list(
    "two_color_path_smooth", ["#FFEE58", "#0D47A1"]
)
...
# ── Plot 1: Full Trial Smooth Path Gradient ─────────────────────────
def generate_full_trajectory_smooth(df, session_dir):
    ...
    # Uniform densification & smoothing
    ...
    ax.set_title(f"{title_str} (Full Trajectory - Smooth Gradient)", fontsize=11, fontweight="bold", pad=20)
    ...
    legend_els = [
        ...
        Line2D([0], [0], color="#0D47A1", lw=3, label="Trajectory End (Dark Blue)"),
    ...
    # Horizontal 2-Color Colorbar showing Path Progress (0% to 100%)
    ...
    cbar.set_label("Trajectory Line Gradient: Start (Yellow) → 50% → End (Dark Blue)", fontsize=9.5, fontweight="bold")
...
# ── Plot 2: Exit Path Smooth Gradient ──────────────────────────────
def generate_exit_trajectory_smooth(df, session_dir):
    plots_dir = os.path.join(session_dir, "plots")
    os.makedirs(plots_dir, exist_ok=True)
    ...
    # Hive marker at original Entry position (green ▲)
    hive_x, hive_y = get_hive_position(df)
    ax.plot(hive_x, hive_y, "o", color="#333333", markersize=9, markeredgecolor="#111111", markeredgewidth=1.2, zorder=10)
    ax.annotate("Hive", (hive_x, hive_y), textcoords="offset points", xytext=(10, 0), fontsize=9, fontweight="bold", color="#333333", va="center", zorder=11)

    title_str, bee_id, outcome = get_metadata(df, session_dir)
    ax.set_title(f"{title_str} (Exit Path - Smooth Gradient)", fontsize=11, fontweight="bold", pad=20)
    ...
    legend_els = [
        Line2D([0], [0], color="#FFEE58", lw=3, label="Feeder Exit Start (Yellow)"),
        Line2D([0], [0], color="#0D47A1", lw=3, label="Arena Exit End (Dark Blue)"),
        Line2D([0], [0], marker="D", color="w", markerfacecolor="#7B1FA2", markersize=8, label="1st Inner Exit (Out)"),
        Line2D([0], [0], marker="o", color="w", markerfacecolor=C_EXIT, markersize=9, label="Exit (Outer)"),
        Line2D([0], [0], marker="o", color="w", markerfacecolor=C_FEEDER, markersize=9, label="Feeder (0,0)"),
    ...
    cbar = fig.colorbar(sm, cax=cax, orientation="horizontal", ticks=[0.0, 0.25, 0.50, 0.75, 1.0])
    cbar.ax.set_xticklabels(["0%", "25%", "50%", "75%", "100%"])
    cbar.set_label("Exit Line Gradient: Feeder Exit (Yellow) → 50% → Exit (Dark Blue)", fontsize=9.5, fontweight="bold")
    cbar.ax.tick_params(labelsize=8)

    fig.savefig(os.path.join(plots_dir, "gradient_exit_trajectory_smooth.png"), dpi=300, bbox_inches="tight")
    plt.close(fig)


def generate_smooth_path_gradient_plots(df, session_dir):
    generate_full_trajectory_smooth(df, session_dir)
    generate_exit_trajectory_smooth(df, session_dir)


def main():
    print("Generating smooth path-length gradient plots (gradient_full_trajectory_smooth.png)...")
    results_dir = "results"
    ...
    for session_name in sorted(os.listdir(results_dir)):
        ...
        try:
            ...
            generate_smooth_path_gradient_plots(df, session_path)
